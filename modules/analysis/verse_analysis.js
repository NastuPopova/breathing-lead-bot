class BreathingVERSEAnalysis {
  ...
  analyzeChildFlow(surveyData) {
    ...
    const personalMessageNew = this.generatePersonalMessage(surveyData, {
      primaryIssue, segment, analysisType: 'child', profile_name: this.getTranslatedChildProfileName(surveyData)
    });
...
generatePersonalMessage(answers, analysisResult) {
  ...
  // === Техника и отзывы ===
    const techniques = {
      adult: {
        ...
        panic_attacks: { name: 'SOS-техника при панике', reviews: ['Паника уходит быстрее', 'Сердцебиение нормализуется', 'Возвращается контроль над собой', 'Страх приступа снижается'], urgency: true },
  ...
  const reviews = tech.reviews || (isChild ? ['Ребёнок становится спокойнее', 'Легче сосредотачивается', 'Улучшается поведение', 'Родители отмечают прогресс'] : ['Уходит напряжение', 'Появляется энергия', 'Улучшается самочувствие', 'Клиенты отмечают быстрый эффект']);
...
Ваше желание помочь — это лучшее, что вы можете дать. Первые результаты родители видят уже через неделю.
...
🧭 Что внутри вашего персонального гида (PDF):
📘 Пошаговая инструкция (текстовые описания шагов)
