...
`;
  ...
  // КУРСЫ ДЫХАТЕЛЬНЫХ ПРАКТИК - АКЦЕНТ НА СТАРТОВОМ
  ...
  message += `👩‍⚕️ *Записаться к Анастасии:* @breathing_opros_bot`;
  ...
  // ДОБАВИТЬ в конец класса PDFBonusManager:
...
async showAllPrograms(ctx) {
  ...
  message += `👩‍⚕️ Записаться: @breathing_opros_bot`;
...
async showOrderDetails(ctx, programType) {
  const programs = {
    ...
    package: {
      ...
      price: '4500₽',
      description: 'Комплексная программа',
  ...
  message += `👩‍⚕️ *Вопросы?* @breathing_opros_bot`;
...
async showProgramHelper(ctx) {
  ...
  message += `🎯 *Персональный подход?* → Консультация (2000₽)\n`;
  message += `💪 *Серьезные изменения?* → Пакет занятий (4500₽)\n\n`;
  message += `💬 Напишите @breathing_opros_bot - поможет определиться!`;
  ...
  await ctx.editMessageText(message, {
    ...
    ...Markup.inlineKeyboard([
      ...
      [Markup.button.url('💬 Спросить эксперта', 'https://t.me/breathing_opros_bot')],
