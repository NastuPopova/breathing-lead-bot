...
`;
  ...
  // Отправка статичных PDF (продолжение)
  ...
  async showProgramHelper(ctx) {
    ...
    message += `💰 До 2500₽ → **Индивидуальная консультация**\n`;
    message += `💎 До 5000₽ → **Пакет из 3 занятий**\n\n`;
    
    message += `❓ *Ваши цели:*\n`;
    message += `🎯 Попробовать и понять → **Стартовый комплект**\n`;
    message += `🔍 Разобрать мою технику → **Индивидуальная консультация**\n`;
    message += `🚀 Серьезные изменения → **Пакет из 3 занятий**\n\n`;
...
const configUpdates = {
  ...
  MAIN_BOT_URL: 'https://t.me/breathing_opros_bot',
  MESSAGES: {
    CONTACT_TRAINER: `📞 *СВЯЗЬ С ТРЕНЕРОМ*\n\n` +
      ...
      `• Заказа курсов и материалов\n\n` +
      `⚡ *Отвечаю в течение 2-4 часов*`,
      
    WELCOME_MESSAGE: `🌬️ *Добро пожаловать!*\n\n` +
      `Я помогу подобрать дыхательные техники под вашу ситуацию.\n\n` +
      `📊 *Пройдите 5-минутную диагностику* и получите:\n` +
      `• Персональный дыхательный гид\n` +
      `• Техники под вашу проблему\n` +
      `• План освоения на 3 дня\n` +
      `• Дополнительные бонусы\n\n` +
      `👩‍⚕️ *Эксперт:* [Анастасия Попова](https://t.me/breathing_opros_bot)`
  }
};
...
const finalLinkCheck = {
  ...
  telegramLinks: '[Анастасии Поповой](https://t.me/breathing_opros_bot)',
  buttonText: 'Записаться к Анастасии',
  buttonUrl: 'https://t.me/breathing_opros_bot'
};

console.log('✅ Все методы программ обновлены с правильными ценами и красивыми ссылками!');
